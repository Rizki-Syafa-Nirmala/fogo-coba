...
<section class="bg-white py-8 antialiased dark:bg-gray-900 md:py-16">
    <div class="mx-auto max-w-screen-xl px-4 2xl:px-0">
      <div class="mx-auto max-w-5xl">
            ...
            <div class="divide-y divide-gray-200 dark:divide-gray-700">
                <div class="grid md:grid-cols-12 gap-4 md:gap-6 pb-4 md:pb-6">
                    <dl class="md:col-span-3 order-3 md:order-1">
                        ...
                        <a href="#" class="hover:underline">{{ $review->food->name }}</a>
                        </dd>
                    </dl>
                    ...
                    <div class="md:col-span-3 content-center order-1 md:order-3 flex items-center justify-between">
                        <dl>
                            ...
                            <dd class="flex items-center space-x-1">
                                @for ($i = 1; $i <= 5; $i++)
                                <svg class="w-5 h-5" fill="{{ $i <= $review->rating ? 'yellow' : 'gray' }}" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        <button id="actionsMenuDropdown{{ $review->id }}"  type="button" class="inline-flex h-7 w-7 items-center justify-center rounded-lg text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white" data-dropdown-toggle="dropdownOrder{{ $review->id }}">
                        ...
                        <div id="dropdownOrder{{ $review->id }}" class="z-10 hidden w-40 divide-y divide-gray-100 rounded-lg bg-white shadow dark:bg-gray-700" data-popper-placement="bottom">
                            <div class="p-2 text-left text-sm font-medium text-gray-500 dark:text-gray-400" aria-labelledby="actionsMenuDropdown">
                                <div>
                                    <button type="button" data-modal-target="editReviewModal{{ $review->id }}" data-modal-toggle="editReviewModal{{ $review->id }}"  class="group inline-flex w-full items-center rounded-md px-3 py-2 text-sm text-gray-500 hover:bg-gray-100 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-600 dark:hover:text-white">
        ...
        <div id="editReviewModal{{ $review->id }}" tabindex="-1" aria-hidden="true" class="fixed left-0 right-0 top-0 z-50 hidden h-[calc(100%-1rem)] max-h-full w-full items-center justify-center overflow-y-auto overflow-x-hidden md:inset-0 antialiased">
            <div class="relative max-h-full w-full max-w-2xl p-4">
                ...
                <div class="relative rounded-lg bg-white shadow dark:bg-gray-800">
                    ...
                    <button type="button" class="absolute right-5 top-5 ms-auto inline-flex h-8 w-8 items-center justify-center rounded-lg bg-transparent text-sm text-gray-400 hover:bg-gray-200 hover:text-gray-900 dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="editReviewModal{{ $review->id }}">
                    ...
                    <!-- Modal body -->
                    <form class="p-4 md:p-5" action="{{ route('review.update', $review->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="col-span-2">
                            <div class="flex items-center flex-row text-5xl">
                                <div class="flex justify-center text-5xl flex-row-reverse">
                                    <!-- 5 Star -->
                                    <input type="radio" id="star5_{{ $review->id }}" name="rating" value="5" {{ $review->rating == 5 ? 'checked' : '' }} class="hidden peer" />
                                    <label for="star5_{{ $review->id }}" class="text-gray-400 cursor-pointer peer-checked:text-yellow-500">&#9733;</label>

                                    <!-- 4 Star -->
                                    <input type="radio" id="star4_{{ $review->id }}" name="rating" value="4" {{ $review->rating == 4? 'checked' : '' }} class="hidden peer" />
                                    <label for="star4_{{ $review->id }}" class="text-gray-400 cursor-pointer peer-checked:text-yellow-500">&#9733;</label>

                                    <!-- 3 Star -->
                                    <input type="radio" id="star3_{{ $review->id }}" name="rating" value="3" {{ $review->rating == 3 ? 'checked' : '' }} class="hidden peer" />
                                    <label for="star3_{{ $review->id }}" class="text-gray-400 cursor-pointer peer-checked:text-yellow-500">&#9733;</label>

                                    <!-- 2 Star -->
                                    <input type="radio" id="star2_{{ $review->id }}" name="rating" value="2" {{ $review->rating == 2 ? 'checked' : '' }} class="hidden peer" />
                                    <label for="star2_{{ $review->id }}" class="text-gray-400 cursor-pointer peer-checked:text-yellow-500">&#9733;</label>

                                    <!-- 1 Star -->
                                    <input type="radio" id="star1_{{ $review->id }}" name="rating" value="1" {{ $review->rating == 1 ? 'checked' : '' }} class="hidden peer" />
                                    <label for="star1_{{ $review->id }}" class="text-gray-400 cursor-pointer peer-checked:text-yellow-500">&#9733;</label>
                                </div>
                            </div>
                        </div>
                        <div class="col-span-2">
                            <label for="comment" class="mb-2 block text-sm font-medium text-gray-900 dark:text-white">Review</label>
                            <textarea id="comment"name="comment" rows="6" class="mb-2 block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary-500 focus:ring-primary-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder:text-gray-400 dark:focus:border-primary-500 dark:focus:ring-primary-500" required="">{{ old('comment', $review->comment) }}</textarea>
                        </div>
                    </div>
                    <div class="border-t border-gray-200 pt-4 dark:border-gray-700 md:pt-5">
                        ...
                        <button type="button" data-modal-toggle="editReviewModal{{ $review->id }}" class="me-2 rounded-lg border border-gray-200 bg-white px-5 py-2.5 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700">Cancel</button>
                    </div>
                    </form>
                </div>
            </div>
